...
BEGIN_MESSAGE_MAP(CcourtDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON11, &CcourtDlg::OnBnClickedCancel)
END_MESSAGE_MAP()
...
void CcourtDlg::OnSelectMainFrame(byte ucUserFrameIndex)			//选择主屏
{
 OnBnClickedCancel();
...
void CcourtDlg::OnBnClickedCancel()
{
	// TODO: 在此添加控件通知处理程序代码

	this->ShowWindow(SW_HIDE);
      //HideAllScreen(true);
	m_pDlgPMosaicScreenPADScreen = new CDlgPMosaicScreenADScreen(this);
	m_pDlgPMosaicScreenPADScreen->Create( CDlgPMosaicScreenADScreen::IDD,this);
	int nFullWidth=1280/*GetSystemMetrics(SM_CXSCREEN)/2*/ ;       
	int nFullHeight=800 /*GetSystemMetrics(SM_CYSCREEN)/2*/ ; 
	m_pDlgPMosaicScreenPADScreen->SetParent(GetDesktopWindow());
	//m_pDlgPMosaicScreenPADScreen->SetWindowPos(&wndTop,0,0,nFullWidth,nFullHeight,SWP_SHOWWINDOW /*| SWP_NOACTIVATE*/);
	m_pDlgPMosaicScreenPADScreen->ShowWindow(SW_SHOWNOACTIVATE);
}
void CcourtDlg::OnP2PData(UINT32 iSessionID, const char * pData, int Len)
{
}